...
import de.neo.smarthome.mobile.api.GroundPlot;
import de.neo.smarthome.mobile.api.IWebAction;
import de.neo.smarthome.mobile.api.IWebLEDStrip;
...
public class ControlSceneActivity extends WebAPIActivity implements ColorPickerDialogListener {
	...
	private void loadControlCenter(Bundle saved) {
				...
				if (values != null && values.length > 0) {
					...
					if (o instanceof GroundPlot) {
						...
						for (Object web : (List<?>) o) {
							if (web instanceof IWebMediaServer.BeanMediaServer) {
								IWebMediaServer.BeanMediaServer ms = (IWebMediaServer.BeanMediaServer) web;
								mRenderer.addMediaServer(ms);
								mParcelableBeans.add(new ParcelBeanMediaServer(ms));
							}
							if (web instanceof IWebSwitch.BeanSwitch) {
								IWebSwitch.BeanSwitch bs = (IWebSwitch.BeanSwitch) web;
								mRenderer.addSwitch(bs);
								mParcelableBeans.add(new ParcelBeanSwitch(bs));
							}
							if (web instanceof IWebAction.BeanAction) {
								IWebAction.BeanAction ba = (IWebAction.BeanAction) web;
								mRenderer.addAction(ba);
								mParcelableBeans.add(new ParcelBeanAction(ba));
							}
							if (web instanceof IWebLEDStrip.BeanLEDStrips) {
								IWebLEDStrip.BeanLEDStrips bs = (IWebLEDStrip.BeanLEDStrips) web;
		...
		if (saved != null && saved.containsKey(GroundPlotKey) && saved.containsKey(Beans)) {
			...
			for (Parcelable bean : mParcelableBeans) {
				if (bean instanceof IWebMediaServer.BeanMediaServer)
					mRenderer.addMediaServer((IWebMediaServer.BeanMediaServer) bean);
				if (bean instanceof IWebSwitch.BeanSwitch)
					mRenderer.addSwitch((IWebSwitch.BeanSwitch) bean);
				if (bean instanceof IWebAction.BeanAction)
					mRenderer.addAction((IWebAction.BeanAction) bean);
				if (bean instanceof IWebLEDStrip.BeanLEDStrips)
					mRenderer.addLEDStrip((IWebLEDStrip.BeanLEDStrips) bean);
			}
			try {
	...
	public void onColorSelected(int dialogId, final int color) {
		if (mSelecter.getBean() instanceof IWebLEDStrip.BeanLEDStrips) {
