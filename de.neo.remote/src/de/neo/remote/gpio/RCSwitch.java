 ...
 * transmitter.switchOn(address, 1); // switches the switch unit A (A = 1, B = 2,
 * 									// ...) on
...
public class RCSwitch {
	 ...
	 *            "1" = on and "0" = off, if all DIP switches are on it's
	 *            "11111")
	 * @param switchCode
	...
	public void switchOn(BitSet switchGroupAddress, int switchCode) {
		...
		this.sendTriState(this.getCodeWordA(switchGroupAddress, switchCode,
				true));
	}
	 ...
	 */
	private String getCodeWordA(BitSet switchGroupAddress, int channelCode,
			boolean status) {
		int nReturnPos = 0;
		char[] sReturn = new char[12];
		String[] code = new String[] { "FFFFF", "0FFFF", "F0FFF", "FF0FF",
				"FFF0F", "FFFF0" };
	...
	public static BitSet getSwitchGroupAddress(String address) {
		if (address.length() != 5) {
			throw new IllegalArgumentException(
					"the switchGroupAddress must consist of exactly 5 bits!");
		}
