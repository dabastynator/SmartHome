...
public class ServerConnection {
	 ...
	 * @throws IOException
	 */
	public synchronized ConnectionSocket getFreeConnectionSocket() throws UnknownHostException, IOException {
		while (true) {
			...
			for (int i = 0; i < serverConnections.size(); i++) {
				...
				if (socket.socket.isClosed()) {
					...
					RMILogger.performLog(LogPriority.WARNING, "Connection closed from: " + serverPort.getIp() + ":"
							+ serverPort.getPort() + "(" + serverConnections.size() + " left)", null);
					continue;
				}
				if (!socket.isInUse()) {
			...
			if (serverConnections.size() < maximumSockets) {
				Socket socket = new Socket(serverPort.getIp(), serverPort.getPort());
				ObjectOutputStream output = new ObjectOutputStream(socket.getOutputStream());
				ObjectInputStream input = new ObjectInputStream(socket.getInputStream());
				ConnectionSocket newSocket = new ConnectionSocket(socket, input, output);
				newSocket.setInUse(true);
				serverConnections.add(newSocket);
				RMILogger.performLog(LogPriority.INFORMATION, "create " + serverConnections.size() + ". connection to: "
						+ serverPort.getIp() + ":" + serverPort.getPort(), null);
				return newSocket;
			}
			try {
	...
	public Object createProxy(String id, Class template, boolean useCache) {
		...
		Object object = Proxy.newProxyInstance(p.getClass().getClassLoader(), new Class[] { template },
				(InvocationHandler) p);
		proxyMap.put(id, object);
		RMILogger.performLog(LogPriority.INFORMATION, "Create new proxy: " + template.getSimpleName(), id);
		return object;
	}
	...
	public class ConnectionSocket {
		 ...
		 * @param output
		 */
		public ConnectionSocket(Socket socket, ObjectInputStream input, ObjectOutputStream output) {
			this.socket = socket;
			this.input = input;
			this.output = output;
			inUse = false;
