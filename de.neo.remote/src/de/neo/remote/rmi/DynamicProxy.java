...
public class DynamicProxy implements InvocationHandler {
	...
	public Object invoke(Object proxy, Method method, Object[] vals) throws Throwable {
		if (method.getName().equals("equals") && vals != null && vals.length == 1)
			...
			try {
				try {
					...
					if (reply == null)
						...
						return sp.createProxy(reply.getNewId(), reply.getReturnType(), !reply.isCreateNewAdapter());
					} else
		...
		if (paramters != null)
			for (int i = 0; i < paramters.length; i++) {
				if (paramters[i] instanceof Proxy) {
					DynamicProxy dp = (DynamicProxy) Proxy.getInvocationHandler(paramters[i]);
					Reply r = new Reply();
					r.addNewId(dp.getId());
					...
					String objId = server.getAdapterObjectIdMap().get(paramters[i]);
					Reply r = new Reply();
					r.setCreateNewAdapter(objId == null);
					if (objId == null) {
						objId = getNextId();
						DynamicAdapter adapter = new DynamicAdapter(objId, paramters[i], server);
	...
	private String getNextId() {
		String id = "newsystem.parameter(" + server.getServerPort() + ":" + (counter++) + ")";
		return id;
	}
