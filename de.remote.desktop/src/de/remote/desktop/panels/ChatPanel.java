  ...
  private Component getControl() {
      ...
      public void actionPerformed(ActionEvent e) {
        try {
          if (ChatPanel.this.inputText.getText().length() > 0)
            ChatPanel.this.server.postMessage(ChatPanel.this.clientName, ChatPanel.this.inputText.getText());
    ...
    this.inputText.addKeyListener(new KeyListener()
    {
      ...
      public void keyPressed(KeyEvent e) {
        if ((e.getKeyCode() == 10) && (ChatPanel.this.inputText.getText().length() > 0)) {
          try {
            ChatPanel.this.server.postMessage(ChatPanel.this.clientName, ChatPanel.this.inputText.getText());
          } catch (RemoteException e1) {
            e1.printStackTrace();
