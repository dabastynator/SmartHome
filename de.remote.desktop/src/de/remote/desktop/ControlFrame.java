...
public class ControlFrame extends JFrame
{
  ...
  public void connectToServer(String registry, int port, String name) {
    try {
      ...
      this.server.connectToRegistry(registry);
      this.server.startServer(port);
      this.station = ((IStation)this.server.find("de.newsystem.idefix.station", 
        IStation.class));
      this.mPlayer = this.station.getMPlayer();
      this.totemPlayer = this.station.getTotemPlayer();
      IBrowser browser = this.station.createBrowser();
      this.mPlayer.addPlayerMessageListener(this.playerListener);
      this.fileBrowser.setBrowser(browser);
      IPlayList playList = this.station.getPlayList();
      this.fileBrowser.setPlayList(playList);
      this.plsBrowser.setPlayList(playList);
      this.chatServer = this.station.getChatServer();
      this.chat.setClientName(name);
      this.chat.setChatServer(this.chatServer);
      setPlayer(this.totemPlayer);
      this.playerMenu.setPlayer(this.totemPlayer, this.mPlayer);
      this.controlMenu.setControl(this.station.getControl());
  ...
  public class DesktopPlayerListener
    implements IPlayerListener
  {
    ...
    public void playerMessage(PlayingBean playing)
    ...
    public void windowClosing(WindowEvent e)
    {
      ...
      try {
        if (ControlFrame.this.currentPlayer != null)
          ControlFrame.this.currentPlayer.removePlayerMessageListener(ControlFrame.this.playerListener);
